...
/**
 * Juan Pedro Martínez Granadoññññññs
 */
public class Proyecto {
    ...
    static boolean rellenarBarcos(char matriz[][], int tamañoBarco)
    ...
    // PRE: Necesario haber reservado memoria de x posiciones en la tabla
    {
        ...
        switch (numero) // Dependiendo del numero aleatorio que toque, el programa se hará de una forma
                        // u otra
        {
    ...
    // PRE: La coordenada pasada como argumento deberá de estar vacia.
    {
        ...
        if (fila >= matriz.length && columna >= matriz[0].length) // Con esta condición miro si la coordenada se sale de
                                                                  // la diagonal inversa.
        {
            dentro = false;
        }
    ...
    static boolean adyacente(char matriz[][], int fila, int columna) {
        ...
        if (dentro(matriz, fila + 1, columna + 1) == true) // Condición para saber si hay adyacencia hacia la diagonal
                                                           // abajo
        {
    ...
    {
        boolean centinela = true; // Variable que usaré para salirme del bucle en cuanto un barco no se pueda
                                  // colocar
        int cont = 10;
        int numerosBarcos[] = { 4, 3, 3, 2, 2, 1, 1 }; // Crearé una tabla donde guardaré la cantidad de barcos a poner
                                                       // en el tablero y el tamaño que ocuparan.
    ...
    static int cantidadBalas(char matriz[][]) // Función que usaré para saber el número de balas de las que dispondrá el
                                              // usuario.
    {
    ...
    // PRE: Es necesario que se hayan puesto todos los barcos aleatorios de manera
    // exitosa.
    {
        int nf = matriz.length;
        int nc = matriz[0].length;
    ...
    // Función que me permite decir desde que columna hasta que columna puede
    // utilizar el usuario
    {
        int col = 65;
    ...
    // Función que me actualiza el juego conforma el usuario va poniendo
    // coordenadas.
    ...
    // POST: Ciertos valores de la matriz del usuario cambiaran.
    {
        ...
        {
            if (comprobarTablero(mUsu, mInterna, fila - 1, columna) == true) // si hay adyacencia compruebo la
        ...
        if (dentro(mUsu, fila, columna - 1) == true) // calculo si la adyacencia hacia la izquierda esta dentro del
                                                     // tablero
        {
            if (comprobarTablero(mUsu, mInterna, fila, columna - 1) == true) // si hay adyacencia compruebo la
                                                                             // coordenada en el tablero.)
            {
                blancos--;
            }
        ...
        if (dentro(mUsu, fila - 1, columna - 1) == true) // calculo si la adyacencia hacia la diagonal arriba esta
                                                         // dentro del tablero
        {
            if (comprobarTablero(mUsu, mInterna, fila - 1, columna - 1) == true) // si hay adyacencia compruebo la
                                                                                 // coordenada en el tablero.)
            {
                blancos--;
            }
    ...
    {
        try {
            new ProcessBuilder("cmd", "/c", "cls").inheritIO().start().waitFor(); // Creo un proceso del sistema
                                                                                  // operativo.
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    // PRE: El usuario deberá de disponer de las balas suficientes para poder usar
    // esta opción.
    {
    ...
    // Función que calcula el tiempo en milisegundos el cual va a estar la matriz
    // interna viendose.
    ...
    // Función que le permite ver al usuario las distintas opciones que tiene para
    // jugar.
    {
        System.out.println(
                "Bomba barrena: Descubre una fila o columna completa con un coste equivalente al tamaño de la fila o columna añadiendole  2 balas");
        System.out.println("Bomba atómica: Descubre una celda y todas sus adyacentes con un coste de 10 balas");
        System.out.println("Pista: Muestra una parte de un barco con un coste de 15 balas");
        System.out.println(
                "Flash: Muestra el tablero completo durante medio segundo y lo oculta inmediatamente, con un coste de 25 balas");
        System.out.println(
                "Bomba criogénica: Descubre una diagonal, ya sea la diagonal principal o la secundaria con un coste de 25 balas. La tabla tiene que tener el mismo número de filas y columnas");
    }

    static int bombaCriogenica(char matrizUsu[][], char matrizInterna[][], int opcion, int cantidadBlancos)
    // Función que utilizo para la opción bomba criógenica, que consiste en mostrar
    // la diagonal principal o secundaría dependiendo de la opción que elija el
    // usuario.
    ...
    public static void main(String[] args) {
        ...
        {
            int cantidadBalas = cantidadBalas(matrizInterna); // Guardo el valor de la cantidad de bala que dispondremos
                                                              // para jugar.

            int cantidadBarcosATocar = cantidadBlancos(matrizInterna); // Variable que usare para contar las veces que
                                                                       // le he dado a los barcos.
            ...
            char c;// Variable que usaré para las columnas. (Evitaré que el usuario se equivoque
                   // convirtiendo las minusculas en mayusculas y solo cogere el primer caracter
                   // del usuario).
            ...
            while (cantidadBalas != 0 && cantidadBarcosATocar != 0) {
                do // Hasta que el usuario no escoja una opción correcta aqui nos quedamos.
                {
                    ...
                    System.out.println(" 6: Manual del usuario");
                    System.out.println(" 7: Bomba criogénica coste: 25 balas");
                ...
                switch (opcionUsuario) {
                    case 1: // Utilizaré este case para un disparo normal.
                        ...
                        do {
                            ...
                            fil = sc.nextInt();
                        } while (fila < 0 || fil > matrizUsu.length - 1); // Con esto me cercioro de que el usuario me
                                                                          // ponga una coordenada válida.
                        do {
                            col = cantidadColumnas(columna); // Uso está función para que el usuario sepa desde que
                                                             // letra hasta que letra va su matriz.
                            System.out.println(" Fila a buscar: " + "de la A " + "a la " + (char) (col - 1));
                            c = sc.next().charAt(0); // Con este método solo cojo el primer caracter que me ponga el
                                                     // usuario, y asi evitar que el programa no funcione.
                            c = Character.toUpperCase(c); // Con este método transformo las minuscúlas en mayúsculas,
                                                          // asi evito que el programa deje de funcionar si el usuario
                                                          // me pone letras minúsculas
                            col = c - 65;
                        } while (fila < 0 || col > matrizUsu[0].length - 1); // Con esto me cercioro que el usuario
                                                                             // ponga una columna válida a buscar.
                    ...
                    case 5: // Opción que usaré para la flash, que me muestra el tablero interno durante x
                            // tiempo.
                        if (cantidadBalas >= 25) {
                        ...
                        break;
                    case 6:
                        manualUsuario(); // Con esta función mostraré el manual de juego al usuario con las distintas
                    ...
                    case 7:
                        if (cantidadBalas >= 25) {
                            if (matrizUsu.length == matrizUsu[0].length) {
                                int opcion;
                                do {
                                    System.out.println("1: Diagonal principal"); // Si el usuario elige esta opción
                                                                                 // mostraré la diagonal principal
                                    System.out.println("2: Diagonal secundaria"); // Si el usuario elige está opción
                                                                                  // mostraré la diagonal secundaria.
                                    opcion = sc.nextInt();
                                } while (opcion <= 0 || opcion < 2); // Hasta que el usuario no ponga una opción válida
                                                                     // aqui se queda.
                                cantidadBarcosATocar = bombaCriogenica(matrizUsu, matrizInterna, opcion,
                                        cantidadBarcosATocar);
                                actualizarInterfaz(matrizUsu, cantidadBalas, cantidadBarcosATocar);
                            } else {
                                System.out.println("No puedo realizar esta opción ya que su matriz no es cuadrada");
                            }

                            cantidadBalas -= 25;
                            actualizarInterfaz(matrizUsu, cantidadBalas, cantidadBarcosATocar);
                        } else {
                            System.out.println("No dispone de balas para usar la opción bomba criogénica");
                        }
                        break;
