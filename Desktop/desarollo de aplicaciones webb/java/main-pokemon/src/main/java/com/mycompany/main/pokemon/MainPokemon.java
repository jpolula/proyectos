    ...
    public static void main(String[] args) 
    {
         ...
         while(pichu.getExp()<75)
        {
         ...
         else //Si Raichu no tiene vida a ganado Pikachu
         {
            ...
            //Parte 2
            Pichu pii=new Pikachu(40);
            Pikachu pica=new Raichu(40);
            System.out.println( pii.equals(pica));
            
            //Parte 3
            Pichu pichus[]=new Pichu[6];
            for(int i=0;i<pichus.length;i++)
            {
                pichus[i]=new Pichu("Cualquiera", (int)(Math.random()*100+1), (int)(Math.random()*100+1));
            }
            
            ComparaSalud c1=new ComparaSalud(); //Nos lo ordena por salud.
            Arrays.sort(pichus,c1);
            for(int i=0;i<pichus.length;i++)
            {
                pichus[i].mostrar();
            }
            
            ComparaEx c2=new ComparaEx();
            Arrays.sort(pichus,c2); //Nos lo ordena por experiencia
