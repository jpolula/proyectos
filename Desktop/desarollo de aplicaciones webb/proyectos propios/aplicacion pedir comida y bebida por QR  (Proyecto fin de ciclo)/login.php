  ...
  <div class="w-full max-w-3xl bg-white/95 border-2 border-[#51B2E0] rounded-2xl shadow-xl p-16">
    ...
    <form action="login.php" method="POST" class="flex flex-col gap-6">
      <div class="relative">
        ...
        <input type="text" id="usuario" name="usuario" required placeholder="Usuario" class="pl-14 pr-4 py-4 w-full rounded-xl border-2 border-[#51B2E0] focus:border-[#51E0E0] text-lg sm:text-xl bg-[#E0FAF4] text-[#2773A5] placeholder-[#51B2E0]/60">
      </div>
      <div class="relative">
        <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#51B2E0] text-3xl">lock</span>
        <input type="password" id="contrasena" name="contrasena" required placeholder="Contraseña" class="pl-14 pr-4 py-4 w-full rounded-xl border-2 border-[#51B2E0] focus:border-[#51E0E0] text-lg sm:text-xl bg-[#E0FAF4] text-[#2773A5] placeholder-[#51B2E0]/60">
      </div>
...
<?php
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $stmt = $conexion->prepare("SELECT * FROM Empleados WHERE Nombre = ? LIMIT 1");
      ...
      if ($resultado && $resultado->num_rows === 1) {
          ...
          // si la contraseña no es correcta ponmemos mensaje de error
          if ($contrasena === $empleado["Correo"]) {
