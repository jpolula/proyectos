<?php
// Script para aplicar el sistema de envío de correos en toda la aplicación
...
<head>
    <meta charset='utf-8'>
    <title>Aplicar sistema de envío de correos en toda la aplicación</title>
    <style>
        body {
...
<body>
    <h1>Aplicar sistema de envío de correos en toda la aplicación</h1>";
...
foreach ($archivos_php as $ruta_completa) {
    ...
    if (strpos($ruta_relativa, 'vendor/') === 0 || 
        ...
        strpos($ruta_relativa, 'aplicar_envio_mail_global.php') !== false) {
        continue;
    }
...
// Verificar la configuración de correo en la base de datos
echo "<h2>Verificando la configuración de correo en la base de datos</h2>";
...
try {
    ...
    if ($tabla_existe) {
        ...
        if ($config) {
            echo "<div class='info'>
                <h3>Configuración de correo actual:</h3>
                <ul>
                    <li><strong>Email remitente:</strong> " . htmlspecialchars($config['email_remitente'] ?? 'No configurado') . "</li>
                    <li><strong>Contraseña:</strong> " . (empty($config['email_password']) ? 'No configurada' : '********') . "</li>
                    <li><strong>Host SMTP:</strong> " . htmlspecialchars($config['email_host'] ?? 'No configurado') . "</li>
                    <li><strong>Puerto SMTP:</strong> " . htmlspecialchars($config['email_puerto'] ?? 'No configurado') . "</li>
                    <li><strong>Seguridad:</strong> " . htmlspecialchars($config['email_seguridad'] ?? 'No configurado') . "</li>
                    <li><strong>Nombre remitente:</strong> " . htmlspecialchars($config['email_nombre_remitente'] ?? 'No configurado') . "</li>
                    <li><strong>Correo activo:</strong> " . ($config['email_activo'] ? 'Sí' : 'No') . "</li>
                </ul>
            </div>";
            
            // Verificar si la configuración está completa
            if (!$config['email_activo'] || empty($config['email_remitente']) || empty($config['email_password']) || empty($config['email_host'])) {
                echo "<div class='warning'>
                    <p>⚠️ La configuración de correo está incompleta o desactivada. Por favor, configure el correo en <a href='admin/configuracion_email.php'>Configuración de Email</a>.</p>
                </div>";
            } else {
                echo "<div class='success'>
                    <p>✅ La configuración de correo está completa y activa.</p>
                </div>";
            }
        } else {
            echo "<div class='warning'>
                <p>⚠️ No se encontraron datos de configuración de correo en la tabla 'configuracion'.</p>
                <p>Por favor, configure el correo en <a href='admin/configuracion_email.php'>Configuración de Email</a>.</p>
            </div>";
        }
    } else {
        echo "<div class='error'>
            <p>❌ No se encontró la tabla 'configuracion' en la base de datos.</p>
        </div>";
    }
} catch (PDOException $e) {
    echo "<div class='error'>
        <p>❌ Error al conectar con la base de datos: " . $e->getMessage() . "</p>
    </div>";
}
...
echo "<div class='info'>
    <p>Para probar el sistema de envío de correos, puede:</p>
    <ol>
        <li><a href='test_envio_mail.php'>Usar la página de prueba</a> para enviar un correo de prueba</li>
        <li>Realizar una reserva completa desde la página principal</li>
        <li>Asegurarse de que la configuración de correo en la base de datos sea correcta</li>
    </ol>
</div>";

echo "<p><a href='index.php' class='btn'>Volver al inicio</a> <a href='test_envio_mail.php' class='btn' style='background-color:#007bff;'>Ir a la página de prueba</a></p>
</body>
</html>";
?>
