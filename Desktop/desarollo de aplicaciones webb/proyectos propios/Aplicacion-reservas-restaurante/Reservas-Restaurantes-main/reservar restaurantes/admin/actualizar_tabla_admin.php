...
try {
    ...
    $stmt = $pdo->query("SHOW COLUMNS FROM administrador LIKE 'email_remitente'");
    $emailRemitenteExists = $stmt->fetch();
    
    if (!$emailRemitenteExists) {
        // Añadir los nuevos campos a la tabla administrador
        $pdo->exec("
        ...
        echo "La tabla de administrador ha sido actualizada con éxito.";
    } else {
        echo "La tabla de administrador ya tiene los campos necesarios.";
    }
    
} catch (PDOException $e) {
    echo "Error al actualizar la tabla de administrador: " . $e->getMessage();
}
?>
